...
size_t AudioDecoder::readSamples(AudioBuffer& buffer, size_t numSamples,
    ...
    if (m_trackInfo.codec.find("msbf") != std::string::npos) {
        ...
        if (ENABLE_BIT_REVERSAL) {
            // Reverse bits in each byte for MSB → LSB conversion
            uint8_t* data = buffer.data();
            
            // Lookup table for fast bit reversal
            ...
            static bool reversalLogged = false;
            if (!reversalLogged) {
                std::cout << "[AudioDecoder] 🔄 Bit reversal applied (MSB → LSB)" << std::endl;
                reversalLogged = true;
            }
        }
